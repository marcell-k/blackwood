...
class HMMEnsembleRegimeDetector:
    ...
    def _preprocess_features_fit(self, features: pd.DataFrame) -> np.ndarray:
        ...
        # Apply EMA smoothing
        features_smooth_df = features.ewm(
    ...
    def _preprocess_features_transform(self, features: pd.DataFrame) -> np.ndarray:
        ...
        combined_smooth = combined.ewm(
            span=self.preprocessing_params_['smooth_span'],
            min_periods=self.preprocessing_params_['min_periods']
        ).mean()
    ...
    def _fit_single_hmm(
        self,
        features_smooth: np.ndarray,
        feature_subset_cols: list[int],
        random_state: int
    ) -> dict | None:
        """
        ...
        if len(likelihood_history) >= 2:
            if likelihood_history[-1] < likelihood_history[-2]:
    ...
    def fit(self, features: pd.DataFrame) -> 'HMMEnsembleRegimeDetector':
        ...
        if self.verbose >= 1:
            status = "Fitting EWM smoother on TRAIN data" if self.use_preprocessing else "Using RAW features (no smoothing)"
            print(f"\nStep 1: {status}")
        ...
        valid_results = [
            ...
            if r is not None 
            and r['converged'] 
            and r['likelihood_stable']
        ]
        
        if len(valid_results) == 0:
            raise RuntimeError(
                "All models failed convergence. Increase n_iter or reduce min_self_prob."
            )
        ...
        n_unstable = sum(
            1 for r in raw_results 
            if r is not None and r['converged'] and not r['likelihood_stable']
        )
    ...
    def predict(
        ...
        if self.verbose >= 1:
            status = "Applying fitted EWM smoother (temporal causality preserved)" if self.use_preprocessing else "Using RAW features (no smoothing)"
            print(f"\nStep 1: {status}")
        ...
        for i, (model, result) in enumerate(zip(self.ensemble_models_, self.ensemble_results_)):
