"""
Wyckoff pattern detection: fractals, zigzag, spring & upthrust signals.
All functions are lookahead-safe with confirmation-bar availability tracking.
"""
...
# ---------------------------------------------------------------------------
# Shared config to keep parameter counts ≤ 5
# ---------------------------------------------------------------------------
@dataclass
class WyckoffConfig:
...
# Helpers
# ---------------------------------------------------------------------------
def _high_low_series(
    df: pd.DataFrame, use_high_low: bool
    ...
    highs = pd.Series(
        ...
        index=df.index, name="HighHL",
    )
    lows = pd.Series(
        np.minimum(df["Close"].values, df["Open"].values),
        index=df.index, name="LowHL",
    )
    return highs, lows


def _build_sorted_events(
    cp: pd.DataFrame, col_value: str, col_avail: str
    ...
    return sorted(
        (int(avail_vals[idx]), int(idx), float(price_vals[idx]))
...
# ---------------------------------------------------------------------------
# Zigzag (numba core unchanged – already O(n) single-pass)
# ---------------------------------------------------------------------------
@njit(cache=True)
def _zigzag_core(
    high: np.ndarray,
...
def calculate_zigzag(
    ...
    zigzag_values, _, pivot_types, pivot_available_at = _zigzag_core(
        ...
        threshold_array, use_high_low=use_high_low,
    )
...
def calculate_confirmed_pivots(
    ...
    fh_value, fl_value, _, _ = calculate_fractals(
        df, left=cfg.fractal_lookback, right=cfg.fractal_lookback,
...
def _scan_wyckoff_structure(
    ...
    for i in range(n):
        ...
        tests_between = [
            (t_idx, t_val) for t_idx, t_val in available_tests
            if p2_idx < t_idx < p1_idx
        ]
    ...
    if is_spring:
        return _pack_spring_output(
            df, signal, limit_price, test_price_out, test_pivot_idx_out,
            s3_price, s2_price, s1_price, s3_idx_out, s2_idx_out, s1_idx_out,
            zone_lower, zone_upper, valid_flag,
        )
...
def _pack_spring_output(df, signal, limit_price, test_price, test_idx,
                         s3p, s2p, s1p, s3i, s2i, s1i, zl, zu, valid):
    out = df.copy()
...
def _pack_upthrust_output(df, signal, limit_price, test_price, test_idx,
                           s3p, s2p, s1p, s3i, s2i, s1i, zl, zu, valid):
    out = df.copy()
