...
from blackwood.indicators.core import calculate_fractals, calculate_zigzag
...
def get_available_pivots_at_bar(
    result_df: pd.DataFrame, bar_index: int, use_combined: bool = True
...
def find_sr_zones(
    ...
    if len(fractal_values) < min_fractals:
        return pd.DataFrame(
            columns=["zone_low", "zone_high", "first_idx", "last_idx", "fractal_count", "bars_since_last"]
        )
    ...
    zones_df = zones_df.sort_values(by=["fractal_count", "cluster_center"], ascending=[False, True]).reset_index(
        drop=True
    )
    ...
    if not result_df.empty:
        result_df = result_df[["zone_low", "zone_high", "first_idx", "last_idx", "fractal_count", "bars_since_last"]]

    return result_df


def _merge_overlapping_zones(zones_df: pd.DataFrame, overlap_threshold: float = 0.7) -> pd.DataFrame:
    """
    ...
    if zones_df.empty:
        return pd.DataFrame(
            columns=["zone_low", "zone_high", "first_idx", "last_idx", "fractal_count", "first_detected_idx"]
        )
    ...
    for idx in range(1, len(zones_df)):
        ...
        if overlap_low < overlap_high:
            ...
            if overlap_ratio >= overlap_threshold:
                ...
                current["fractal_count"] = max(current["fractal_count"], candidate["fractal_count"])
                current["first_detected_idx"] = min(
                    current["first_detected_idx"], candidate.get("detection_idx", candidate["first_idx"])
                )
                continue
...
def find_sr_zones_rolling(
    ...
    if isinstance(step, str):
        resampled_idx = df.resample(step, label="left", closed="left").first().index

        detection_indices = df.index.get_indexer(resampled_idx, method="ffill")
        detection_indices = detection_indices[(detection_indices >= window - 1) & (detection_indices < n_bars)]
    else:
    ...
    if show_progress:
        try:
            ...
            detection_iterator = tqdm(
                detection_indices, desc="Detecting S/R zones", unit="bar", total=len(detection_indices)
            )
        except ImportError:
    ...
    for i in detection_iterator:
        ...
        if not zones.empty:
            ...
            if expiration_bars is not None:
                zones = zones[zones["expires_at_time"].isna() | (zones["expires_at_time"] > current_time)].copy()
...
def add_in_zone_column(
    ...
    df["in_zone"] = np.select(
        condlist=[conflict_mask, only_below_mask, only_above_mask], choicelist=[2, -1, 1], default=0
    ).astype(np.int8)
...
def check_in_zone_at_bar(
    ...
    active_mask = (zones["first_detected_idx"] <= current_bar_idx) & (zones["expires_at_idx"] > current_bar_idx)
    ...
    entry_from_below = (previous_close < zone_lows) & (current_close >= zone_lows) & (current_close <= zone_highs)
    gap_through_up = (previous_close < zone_lows) & (current_close > zone_highs)
    from_below_triggers = entry_from_below | gap_through_up

    entry_from_above = (previous_close > zone_highs) & (current_close >= zone_lows) & (current_close <= zone_highs)
