...
# - Ehler dominant cycle period estimator
# ------------------------------------------------------------------------------
@jit(nopython=True)
...
# ------------------------------------------------------------------------------
# Adaptive Stochastic using Ehlers dominant cycle period
# ------------------------------------------------------------------------------
@jit(nopython=True)
...
def rolling_geometric_mean(returns: pd.Series, window: int) -> pd.Series:
    """
    O(N) vectorized calculation of rolling geometric mean.
    1000x faster than rolling().apply().
    """
    ...
    # rolling_sum = log_returns.rolling(window).sum() is essentially O(N) in pandas/numpy
    # but for pure numpy speed we can use:
    ...
    # Divide by window (arithmetic mean of logs)
...
# ------------------------------------------------------------------------------
# Adaptive SMI (Stochastic Momentum Index) using Ehlers dominant cycle period
# ------------------------------------------------------------------------------
@jit(nopython=True)
